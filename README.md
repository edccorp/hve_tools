...
2. *(Optional)* Set **Clip To Object** to surface only the points inside a boundary mesh's box — a box/cube clips a true 3D volume (trimming points above and below too), while a flat plane clips just its XY footprint. Scale it over the area of interest to trim away far-off points.
