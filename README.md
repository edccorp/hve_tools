...
2. *(Optional)* Set **Clip To Object** to surface only the points inside a boundary mesh's convex XY footprint — scale a plane over the area of interest to trim away far-off points.
3. *(Optional)* Under **Pre-filter**, enable **Subsample (Voxel)** and/or **Remove Outliers (SOR)** to clean the cloud before surfacing. Filters run per-click and never modify the source; **Filter To New Point Cloud** saves a filtered copy without surfacing. The texture always bakes from the full, unfiltered cloud, so filtering thins only the geometry.
